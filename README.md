...
| **`deploy/`** | Пример nginx, **`README.md`** и **`rsync-deploy.sh.example`** — как выкатывать только `web/` + `api/`. |
| **`client/`** | Заготовка под клиентскую часть (Android и т.д.). |
| **`docs/`** | Документация. |
...
Открыть http://localhost:8080/ — статика и PHP из `web/`, пути `/api/...` отдаются из `api/`.
...
См. **`deploy/README.md`**: варианты **rsync** (скрипт-пример), **git pull** на сервере, CI/CD. На прод попадают только **`web/`** и **`api/`**; `config.json` и логи на сервере по умолчанию не перезаписываются.
