...
На прод нужны **`web/`** и **`api/`**; **`client/`** — чтобы с ригов Hive OS работал `wget https://…/client/install.sh`. Папки `docs/`, примеры nginx — не обязательны для работы сайта.

## Варианты

### 1. `rsync` с ПК (просто и прозрачно)

Скрипт-пример: **`rsync-deploy.sh.example`** — копирует только `web/` и `api/`, не трогает `config.json` на сервере.
