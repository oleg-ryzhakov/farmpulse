...
**Один запрос** `stats` (как у таймера). В режиме `trace` ответ **только показывается** — reboot/exec **намеренно не выполняются**, чтобы при отладке не уронить риг. Рабочий таймер `farmpulse-sidecar` команды **выполняет**.
